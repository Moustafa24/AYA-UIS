  ...
  const handlePromoteStudent = async e => {
    ...
    try {
      ...
      toast.success(
        res?.message ||
          res?.data?.message ||
          `Student ${academicCode} promoted successfully!`
      );
      setAcademicCode('');
    } catch (err) {
      toast.error(
        ...
        >
          ...
          <div>
            ...
            <p style={{ color: 'var(--text-light)', fontSize: '0.85rem' }}>
              Promote all non-graduated students to the next year level. This
              will move students from preparatory to 1st year, 1st to 2nd, 2nd
              to 3rd, and 3rd to 4th year.
            </p>
          </div>
        </div>
        <button
        ...
        >
          {promoteAllLoading ? (
            <>
              <span
                className="spinner"
                style={{ width: 16, height: 16, margin: 0, borderWidth: 2 }}
              />
              Promoting...
            </>
          ) : (
