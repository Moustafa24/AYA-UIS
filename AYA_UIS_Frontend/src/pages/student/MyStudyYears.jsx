  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h1><FiCalendar style={{ marginRight: 8 }} />My Study Years</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 20 }}>
          {studyYears.map(sy => (
            <div
              ...
              onClick={() => navigate(`/student/study-year/${sy.studyYearId || sy.id}/semesters`)}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <div>
                  <h3 style={{ fontSize: '1.1rem', marginBottom: 4 }}>
                    {sy.startYear || sy.studyYear?.startYear} — {sy.endYear || sy.studyYear?.endYear}
                  </h3>
                  <p style={{ color: 'var(--text-light)', fontSize: '0.85rem', marginBottom: 8 }}>
                    {LEVEL_LABELS[sy.level || sy.levelName] || sy.levelName || sy.level || '—'}
                  </p>
                  {sy.departmentName && (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {sy.isCurrent && <span className="badge badge-info">Current</span>}
